...
export class CreateAttendanceService implements ICreateAttendance {
    ...
    async create({ cpf, name, service, queue_type }: IAttendanceModel): Promise<AttendanceEntity> {
        const attendance = new AttendanceEntity(cpf, name, service, queue_type)
        const tenMinutesAgo = new Date(Date.now() - 10 * 60 * 1000)
        const alreadyExistAnAttendance = await this.repo.findByCpf(attendance.cpf, tenMinutesAgo)

        if (alreadyExistAnAttendance) throw new Error('Já existe um atendimento para este usuário nos últimos 10 minutos.')

        const numberOfAttendances = await this.repo.countByServiceAndQueueType(attendance.service, attendance.queue_type)
